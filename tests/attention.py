...
class AttentionTestCase(unittest.TestCase):
    def _test_attention(
        ...
        hidden_states_ait = hidden_states.clone().to(
            hidden_states.device, hidden_states.dtype
        )
        if len(hidden_shape) == 4:
            hidden_states_ait = hidden_states_ait.permute(0, 2, 3, 1).contiguous()
        encoder_hidden_states_ait = (
        ...
        op_ait = attention_processor.Attention(
            query_dim=query_dim,
            cross_attention_dim=cross_attention_dim,
            heads=heads,
            dim_head=dim_head,
        ...
        if Encoder_hidden_states is not None and Attention_mask is not None:
            Y = op_ait.forward(Hidden_states, Encoder_hidden_states, Attention_mask)
        elif Encoder_hidden_states is not None:
            Y = op_ait.forward(Hidden_states, Encoder_hidden_states)
        elif Attention_mask is not None:
            Y = op_ait.forward(Hidden_states, attention_mask=Attention_mask)
        else:
            Y = op_ait.forward(Hidden_states)
        Y = mark_output(Y, "Y")
        ...
        inputs_dict = {"Hidden_states": hidden_states_ait}
        if Encoder_hidden_states is not None:
            inputs_dict["Encoder_hidden_states"] = encoder_hidden_states_ait
        if Attention_mask is not None:
            inputs_dict["Attention_mask"] = attention_mask_ait
