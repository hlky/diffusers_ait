...
class SpatialNormTestCase(unittest.TestCase):
    def _test_spatial_norm(
        ...
        f_ait = f.clone().permute(0, 2, 3, 1).contiguous().to(f.device, f.dtype)
        zq_ait = zq.clone().permute(0, 2, 3, 1).contiguous().to(zq.device, zq.dtype)
