...
parser.add_argument("--min-frames", type=int, default=9)
parser.add_argument("--max-frames", type=int, default=61)
parser.add_argument("--min-batch", type=int, default=1)
parser.add_argument("--max-batch", type=int, default=1)
parser.add_argument("--hf_hub", type=str, default="hunyuanvideo-community/HunyuanVideo")
...
def map_vae(pt_module, device="cuda", dtype="float16", encoder=False):
    ...
    for key, arr in pt_params.items():
        ...
        if (
            ...
            params_ait[key] = torch.permute(arr, [0, 2, 3, 4, 1]).contiguous()
        else:
            params_ait[key] = arr
    if encoder:
...
hf_hub = args.hf_hub
model_name = f"autoencoder_kl_hunyuan_video.decoder.{resolution[1]}.{device_name}.sm{sm}"
...
batch_size = IntVar([batch_size[0], batch_size[1]], name="batch_size")
frames = 1
height = IntVar([height[0] // 8, height[1] // 8], name="height")
width = IntVar([width[0] // 8, width[1] // 8], name="width")
